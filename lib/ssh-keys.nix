{
  sadbeast = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINGpEusv/bS34Q1JQxZXikdcwnq1vToz2d+HgV+E8NRX"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFhHC9V56/Vy7NF9xs0zfxhg3AH/pkDr7iIFyafYpwQR termux"
    "ssh-rsa  anroid"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIH4RdAvR+tb35UWiAnKnBjDxEwK2JmLGsw5SUdUAmcqS android ps"
  ];

  kent = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINGpEusv/bS34Q1JQxZXikdcwnq1vToz2d+HgV+E8NRX"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFhHC9V56/Vy7NF9xs0zfxhg3AH/pkDr7iIFyafYpwQR termux"
    "ssh-rsa  anroid"
  ];

  # Guests allowed into the shared `pair` account on pairvm only
  # (microvms/pairvm.nix). Never reference this list from a host account.
  pairGuests = [];

  # Primary key used for admin access
  primary = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINGpEusv/bS34Q1JQxZXikdcwnq1vToz2d+HgV+E8NRX";
}
