{
  ...
  # Stylix target: paint every herdr chrome token from the base16 scheme so
  # the sidebar matches the terminal. Installed in place of herdr-radar's
  # theme block (radar only sets active_row_bg there, and treats a
  # `[theme.custom]` it did not write as user-owned and leaves it alone).
  stylixEnabled = config.stylix.targets.herdr.enable;
  stylixThemeBlock = pkgs.writeText "herdr-theme-stylix.toml" (with config.lib.stylix.colors.withHashtag; ''
    # Generated by Stylix
    [theme.custom]
    accent = "${base0D}"
    panel_bg = "${base00}"
    sidebar_bg = "${base00}"
    active_row_bg = "${base02}"
    selection_bg = "${base02}"
    surface0 = "${base01}"
    surface1 = "${base02}"
    surface_dim = "${base01}"
  ...
  herdrConfig =
    pkgs.runCommand "herdr-config.toml" {
      ...
      ${lib.optionalString stylixEnabled ''
        awk -v blk="$(cat ${stylixThemeBlock})" '
          /^# >>> herdr-radar theme block/ { print blk; skip = 1; next }
          /^# <<< herdr-radar theme block/ { skip = 0; next }
          !skip' "$out" > "$out.tmp" && mv "$out.tmp" "$out"
      ''}
    '';
...
in {
  ...
  options.stylix.targets.herdr.enable = lib.mkOption {
    type = lib.types.bool;
    default = (config.stylix.enable or false) && (config.stylix.autoEnable or false);
    description = "Whether to enable theming for herdr.";
  };
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf config.programs.claude-code.enable {
      ...
      home.activation.herdrClaudeHook = lib.hm.dag.entryAfter ["writeBoundary"] ''
        ...
        [ -f "$settings" ] || { mkdir -p "${claudeDir}" && printf '{}\n' > "$settings"; }
